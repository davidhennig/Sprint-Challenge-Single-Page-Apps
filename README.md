...
When creating a webpage there are two options for where the majority of the sites content is stored. You could either have the content stored on the Server-Side, or on the Client-Side. These each have their own benefits and drawbacks.

Storing information on the Server-Side the initial start up of a page or reloading the page after performing some type of action will take longer because it has to transmit all of the information back to the server and then back to the client. This does, however, leave more room on the Client-Side and makes performing actions on the Client-Side faster.

When storing information on the Client-Side taking all the information from the server is downloaded to the client to house the site on the client. The initial download of
information from the server will take a longer period of time as it is gathering a large amount of data. This allows easier and faster actions to take place though, because the actions can be performed on the client and doesn’t need to reload its entirety from the server each time.
